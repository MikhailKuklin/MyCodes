...
cd turbogap/t01
ls -1v template* | grep 'template*' > list
sed -i 's/://g' list
...
cd ../../
cd training/
ls -1v template* | grep 'template*' > list2
sed -i 's/://g' list2
sed -i '1 i\vasp' list2
head -n -1 list2 > list_ds
rm list2
for i in $(cat list_ds); do cd $i; grep "default_sigma" training_forces.sh; cd ../; done > defsigma
awk '{print $1}' defsigma > defsigma2
sed -i 's/default_sigma={//' defsigma2
rm defsigma
mv defsigma2 ./t01
cd t01
ls -1v step* | grep 'step*' > list2
sed -i 's/://g' list2
sed -i '1 i\vasp' list2
head -n -1 list2 > list
rm list2
